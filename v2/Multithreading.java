...
class Multithreading extends Thread {
  ...
  final String header_without_nonce = Mine.block.get("version") + Mine.block.get("previous_block_hash") + Mine.block.get("merkle_root") + Mine.block.get("time") + Mine.block.get("bits");
  ...
  // final int target_trailing_zeros = get_num_trailing_zeros(Mine.block.get("target"));
  final int target_leading_zeros = get_num_leading_zeros(Mine.block.get("target"));
  ...
  */
  ...
  public void run() {
    ...
    while (true) {
      ...
      if (num_leading_zeros >= target_leading_zeros) {
        try {
          // Ask the Python Mining Client to submit a hash
          String command = "python3 mining_client.py" + " " + "submit_results" + " " + Mine.address + " " + Mine.host + " " + Mine.port + " " + Mine.block.get("job_id") + " " + Mine.block.get("extranonce2") + " " + Mine.block.get("time") + " " + nonce;
      ...
      if (hashes_since_last_report % report_frequency == 0) {
        ...
        duration = (System.currentTimeMillis() - Mine.start_time) / 1000.0;
